...
2026-09-14, compiled and confirmed working by the user:** no longer
...
**The office hub (`rm003.sc`, `OFFICE_ROOM`): built and compiled
2026-09-14, dialogs confirmed working by the user.** Designed with the user so
...
question that can always be backed out of, via a visible "Not yet"
button (`PrintChoices` ignores Escape, see Findings). New players
confirm a standard 10-turn
...
- There are two sources now: `text/office.txt` → `text.003`
  (`TEXT_OFFICE`) and `text/menu.txt` → `text.010` (`TEXT_MENU`, the menu
  bar's Help screen). Room text follows Sierra's convention of matching
  its room number, which means `text.003` also shares a number with
  `rm003.sc`'s SCRIPT, harmless as long as nothing ever calls
  `DisposeScript()` on a TEXT define (see Findings for the `TEXT_UI` 0 /
  `Main.sc` 0 bug that rule comes from). Text that isn't tied to a room,
  like `text.010`, just avoids script numbers altogether.
...
- **A dialog's Escape comes back as 0/-1, not a button, and choice
  ...
  `doit()`'s -1 to 0 and then fell through its button-match loop
  unchanged, so Escape during an event applied that event's FIRST choice,
  effects and all; Escape on the portrait picker likewise returned -1 as
  if it were an appearance. Both now rebuild the same page when nothing
  matched, Escape is ignored outright, and any dialog that wants a way
  out gives it a real button with its own value (rm003.sc's "Not yet").
  `ShowCaseFiles` was already safe: its values start at 1, so 0 is an
  unambiguous "cancelled". Worth checking in any new hand-built `Dialog`:
  a `doit()` return of 0 is indistinguishable from a button whose value
  is 0.
...
7. **Office hub: compiled 2026-09-14, dialogs confirmed by the user.**
   ...
   **Imperative Verb** class it needs in the Vocabulary editor. The user
   ...
   - Survive a run, then click the computer → Standard / Extended / Not
     yet. Escape should do nothing there (it rebuilds the prompt), and
     Escape during an event must not pick a choice.
...
8. ~~Menu-bar boilerplate~~ **done, confirmed working by the user.**
   ...
   and credits "by Andrew Edmark". `Help` (`F1`) now
   prints a real how-to-play from `TEXT_MENU` (`text/menu.txt` →
   `text.010`, see the text pipeline above), split across five short
   dialogs shown one after the other, one paragraph each: a turn, how a
   session ends, the office, typing, and the hotkeys. Titled "How To Play
   (N of 5)", with a "More" button on the first four and "OK" on the
   last. The split is load-bearing, not cosmetic: `Print()` never
   paginates (it sizes one dialog to the whole string, and only
   `DSelector` scrolls), a dialog that overflows the 200px screen renders
   garbled rather than clipping (the same failure `PrintChoices` clamps
   against), and a blank line inside a dialog reads badly. Each entry is
   roughly 200 characters now; for scale, the longest string the game
   already prints this way is ~150, an event prompt in `rm200.sc`. To
   adjust, edit `text/menu.txt` and re-run the generator. Adding or
   removing a paragraph does need a matching `Print()` call in
   `menubar.sc`.
